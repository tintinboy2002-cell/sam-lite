...
const Family = () => {
  ...
  const getContacts = async () => {
    ...
    try {
      const response = await httpInjectorService.getFamilyContact(id);
  ...
  const handleCloseModal = (pri_contact_id) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      const payload = { pri_contact_id: deleteId.pri_contact_id, user_id: id };
  ...
  return (
    <React.Fragment>
      {/* <Card className="shadow p-1 bg-white mb-3" style={{ marginTop: '20px' }}> */}
        <Box className="card-header" mb={4}>Employee Emergency Contacts</Box>
