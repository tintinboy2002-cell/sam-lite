...
const Education = () => {
  ...
  const [initialForm, setInitialForm] = useState(defaultForm(userId));
  ...
  const fetchEducationData = async () => {
    setLoading(true);
    try {
      const response = await httpInjectorService.getEducationDetails(userId);
  ...
  const openAddModal = () => {
    setInitialForm(defaultForm(userId));
    setFileAttachment([]);
    setEditMode(false);
    setEditingId(null);
    onOpen();
  ...
  //   setFileAttachment((prev) => prev.filter((_, i) => i !== index));
  ...
  const submitEducation = async (values, { resetForm }) => {
    ...
    const payloadBase = { ...values, user_id: userId };
    ...
    <>
      <Box>
        <HStack justify="space-between" mb={6}>
          ...
          >
            <AddIcon />
            <Text mb="0px" display={{ base: 'none', md: 'block' }}>
              Add Education
            </Text>
          </Button>
        </HStack>
