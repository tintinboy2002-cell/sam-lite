...
const MonthlyLogs = ({ activeTab }) => {
  ...
  // console.log(employeeMonthlyLogs, 'employeeMonthlyLogs');
 ...
 const submitRaiseRequest = async () => {
  try {
    const subtractISTOffset = (time) => {
      if (!time) return null;
      let [hours, minutes] = time.split(":").map(Number);

      // Convert to total minutes and subtract 330 (5 hours 30 mins)
      ...
      // Format back to HH:mm (zero-padded)
      return `${String(newHours).padStart(2, "0")}:${String(newMinutes).padStart(2, "0")}`;
    };

    const payload = {
      date: formatDateForBackend(date),
      clock_in_time: subtractISTOffset(clockIn),
      clock_out_time: subtractISTOffset(clockOut),
      reason,
  ...
  // Utility to format date range payload
  const getFormattedRange = (dateRange) => ({
    fromDate: dateRange.fromDate ? formatLocalDate(dateRange.fromDate) : null,
    toDate: dateRange.toDate ? formatLocalDate(dateRange.toDate) : null,
  });

  // Main function to fetch data (for both admin and user)
  const fetchMonthlyLogs = async (dateRange) => {
    setLoading(true);
    try {
      const formattedRange = getFormattedRange(dateRange);
      let response;

      if (roleId === 2) {
        // Admin API
        response = await httpInjectorService.adminmonthlylogs(formattedRange);
        console.log('admin api called', response);
      } else if (roleId === 3) {
        // Employee API
        response = await httpInjectorService.monthlylogs(formattedRange);
        console.log('employee api called', response);
      } else {
        console.warn('Unknown role, skipping API call');
        return;
      }
  ...
  useEffect(() => {
    if (activeTab === '2') {
      ...
      const range = { fromDate: twoDaysAgo, toDate: today };
      setDateRange(range);
  ...
  const columns = useMemo(
    () => [
...

      {
        ...
        id: 'actions',
        Cell: ({ row }) => {
          const { formattedClockIn, formattedClockOut, approval_status } =
            row.original;

          const isClockInEmpty = !formattedClockIn || formattedClockIn === '--';
          ...
          const raiseCompleted = rowKey ? completedRows[rowKey]?.raise : false;
          const leaveCompleted = rowKey ? completedRows[rowKey]?.leave : false;

          const isRejected = approval_status?.toLowerCase() === 'rejected';
          const isApproved = approval_status?.toLowerCase() === 'approved';

          // If approved, skip actions (dates handled automatically)
          if (isApproved) {
            return null;
          }
          return (
          ...
          {loading ? (
            ...
            <Box position="relative">
              {employees.length > 0 && (
                <>
                  <TableContainer
                    key={JSON.stringify(completedRows)}
                    columns={columns}
                    data={employees}
                    isGlobalFilter={true}
                    customPageSize={10}
        ...
        <ModalContent>
          ...
          <ModalBody>
            <FormControl mb={4}>
              ...
              <Input type="text" value={date} isDisabled />
            </FormControl>
            ...
            <FormControl isInvalid={reason && reason.trim().length < 5} mb={4}>
